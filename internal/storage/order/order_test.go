...
func TestRepository_Update(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := res.Update(tt.args.order); (err != nil) != tt.wantErr {
				t.Errorf("Repository.Create() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
...
func TestRepository_FindByUserID(t *testing.T) {
	...
	res.Create(&model.Order{
		...
		Status:     "New",
		Accrual:    0,
		UploadedAt: "2020-12-10T16:12:01+03:00",
	})
	type args struct {
		userID int
	}
...
func TestRepository_FindByNumber(t *testing.T) {
	db := InitDB("postgresql://test:test@127.0.0.1:5433/test?sslmode=disable")
	res := NewRepository(db)
	res.Migrate()
	defer res.Drop()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := res.FindByNumber(tt.args.number)
			if (err != nil) != tt.want.wantErr && tt.want.err != "" {
				if err.Error() != tt.want.err {
